#!/usr/bin/env python
"""
Collect all JSON files from nested directories into a single output directory.
...
def collect_json_files(
    ...
    """
    Collect all JSON files from input directory tree to output directory.
    ...
    for json_file in tqdm(json_files, desc="Collecting files"):
        # Generate output filename
        output_name = generate_output_filename(json_file, input_path, preserve_structure)
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect all JSON files from nested directories into a single output directory"
    )
    ...
    if args.move and not args.dry_run:
        response = input(f"This will MOVE files from {args.input_dir}. Continue? (y/n): ")
        if response.lower() != 'y':
            print("Aborted")
            return
    ...
    if not args.dry_run:
        ...
        print(f"Files {'moved' if args.move else 'copied'}: {processed}")
        if skipped > 0:
